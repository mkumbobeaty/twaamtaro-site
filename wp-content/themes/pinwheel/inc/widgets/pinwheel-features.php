<?php
if ( ! class_exists( 'Pinwheel_Features' ) ) :

class Pinwheel_Features extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'pinwheel_features_widget', 'description' => __( 'Show Features .', 'pinwheel') );
        parent::__construct(false, $name = __('Pinwheel: Features', 'pinwheel'), $widget_ops);
		$this->alt_option_name = 'pinwheel_features_widget';
	...
	if ( ! isset( $args['widget_id'] ) ) {
		...
		$category = isset( $instance['category'] ) ? esc_attr($instance['category']) : absint( 0 );
		$subtitle = isset( $instance['subtitle'] ) ? esc_attr($instance['subtitle']) : '';
		$number = isset( $instance['number'] ) ? esc_attr($instance['number']) : absint( 6 );
